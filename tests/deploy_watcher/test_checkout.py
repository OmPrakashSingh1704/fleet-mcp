...
from flotilla_mcp.deploy_watcher.checkout import CheckoutError, sync_checkout
...
def test_sync_checkout_rejects_option_shaped_or_malformed_sha_without_invoking_git(tmp_path, bad_sha):
    ...
    with patch("flotilla_mcp.deploy_watcher.checkout.subprocess.run") as mock_run:
        with pytest.raises(CheckoutError):
...
def test_clone_and_fetch_use_env_reading_credential_helper_without_token_in_argv(tmp_path, monkeypatch):
    ...
    from flotilla_mcp.deploy_watcher import checkout
    ...
    with patch("flotilla_mcp.deploy_watcher.checkout.subprocess.run", side_effect=ok) as mock_run:
...
def test_checkout_error_strips_userinfo_and_token(monkeypatch, tmp_path):
    ...
    with patch("flotilla_mcp.deploy_watcher.checkout.subprocess.run", side_effect=fail):
        with pytest.raises(CheckoutError) as exc_info:
